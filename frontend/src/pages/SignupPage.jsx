...
const SignupPage = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      // If it's disabled, the user is logged in automatically.
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full animate-fade-in-up">
        ...
        <div className="text-center mb-10">
          ...
          <p className="mt-3 text-slate-500 font-medium">Start your forensic journey today</p>
        </div>
        ...
        <div className="bg-[#1e293b] rounded-3xl border border-slate-700/50 shadow-2xl overflow-hidden">
          <div className="p-8 sm:p-12">
            {success ? (
              <div className="text-center space-y-6 py-4">
                <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-teal-400/10 border border-teal-400/20 mb-4">
                  <span className="text-4xl">🎉</span>
                </div>
                <h3 className="text-2xl font-bold text-white">Account created!</h3>
                <p className="text-slate-400">We've sent a confirmation email to <span className="text-white font-medium">{email}</span>. Please verify your email to log in.</p>
                <button
                  onClick={() => navigate('/login')}
                  className="w-full flex justify-center py-4 px-4 border border-transparent text-sm font-black rounded-2xl text-white bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-500 hover:to-emerald-500 transition-all duration-200 shadow-lg shadow-teal-500/20 uppercase tracking-wider"
                >
                  Go to Login
                </button>
              </div>
              ...
              <>
                <div className="mb-10 text-center">
                  ...
                  <p className="text-slate-400 mt-2">Join the next generation of forensics</p>
                </div>
                ...
                <div className="mt-8">
                  <div className="relative">
                    ...
                    <div className="relative flex justify-center text-sm">
                      <span className="px-4 bg-[#1e293b] text-slate-500 font-medium">or join with</span>
                    </div>
                  </div>
          ...
          <div className="bg-slate-800/30 p-6 text-center border-t border-slate-700/30">
            <p className="text-sm text-slate-400">
              ...
              <Link to="/login" className="font-bold text-indigo-400 hover:text-indigo-300 transition-colors underline decoration-indigo-400/30 underline-offset-4">
                Log in here
              </Link>
            </p>
          </div>
